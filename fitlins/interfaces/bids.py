...
class LoadBIDSModel(SimpleInterface):
    ...
    def _load_graph(self, runtime, graph, node=None, inputs=None, **filters):
        ...
        step_subdir = Path(runtime.cwd) / node.name
        ...
        for child in node.children:
            all_specs.update(self._load_all_specs(
                runtime,
                graph,
                child.destination,
                outputs,
    ...
    def _load_run_level(runtime, graph, specs):
        design_info = []
        warnings = []
        ...
        for spec in specs:
            ...
            if "RepetitionTime" not in spec.metadata:
                ...
                raise ValueError(
                    "Preprocessed file %s does not have an associated RepetitionTime"
                    % fname)

            info["repetition_time"] = spec.metadata['RepetitionTime'][0]

            ent_string = '_'.join('{}-{}'.format(key, val)
                                  for key, val in spec.entities.items())
...
￼           # the BIDS model.
            ...
            spec.data.to_hdf(dense_file, key='dense')
